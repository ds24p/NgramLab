        ...
        ui.tags.script(
            ...
            document.addEventListener("DOMContentLoaded", function () {
                ...
                document.addEventListener("click", function (event) {
                    ...
                    if (target.closest(".route-nav .nav-link")) {
                        setMobileNavOpen(false);
                    }
...
if RedirectResponse is not None and Route is not None:
    def route_redirect(request):
        route_path = normalize_route_path(request.url.path)
        return RedirectResponse(url=f"/#{route_path}", status_code=302)
    ...
    redirect_routes.extend(
        Route(alias, route_redirect, methods=["GET"])
    ...
    app.starlette_app.routes[static_mount_index:static_mount_index] = redirect_routes
